...
Value LineParser::ExecuteFunctionCall(const std::string& functionName, const std::vector<Value>& args)
{
	...
	// Using separate function scope system
	int functionScopeId = -1;
	int functionScopeCount = -1;
	
	// Push a new function scope (independent of FOR stack)
	m_sourceCode->OpenFunctionScope(m_line, m_column);
	
	// Also open a brace to align FOR stack so that local variables work correctly
	// This is needed because Process() uses GetScopedSymbolName which relies on FOR stack
	...
	// Set up parameter bindings in the symbol table at current function scope level
	...
	m_sourceCode->CloseFunctionScope(m_line, m_column);
